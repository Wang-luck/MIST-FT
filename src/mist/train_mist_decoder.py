""" train_mist_decoder.py

只微调 FPGrowingModule（解码器/输出头），其余全部冻结。
"""
import yaml
import logging
import pickle
...
def run_training():
    ...
    # 只训练输出头（FPGrowingModule），其余全部冻结
    for name, param in model.named_parameters():
        param.requires_grad = "spectra_encoder.2" in name

    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total = sum(p.numel() for p in model.parameters())
    logging.info(f"Trainable (decoder only): {trainable:,} ({trainable/total*100:.1f}%)")
