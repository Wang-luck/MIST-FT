""" train_mist_feature.py

只训练 pairwise_featurizer（峰对特征），其余全部冻结。
用于测试中药特有碎裂模式是否能通过峰对特征学到。

用法:
    python3 src/mist/train_mist_feature.py \
...
def run_training():
    ...
    # 只训练 pairwise_featurizer，其余全部冻结
    for name, param in model.named_parameters():
        if "pairwise_featurizer" in name:
    ...
    logging.info(f"Trainable: {trainable:,} ({trainable/total*100:.2f}%)")
    logging.info(f"Frozen: {total - trainable:,}")
