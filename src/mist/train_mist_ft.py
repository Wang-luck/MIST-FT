""" 
选择性微调：冻结化学式嵌入和早期MLP，训练注意力层和输出头。
适合目标数据与预训练数据分布不同（如中药vs代谢物）的场景。
"""
import yaml
import logging
import pickle
...
def get_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Selective fine-tuning: freeze embedding+MLP, train attention layers + output head",
    )
...
def run_training():
    ...
    #  Selective fine-tuning: freeze embedding+early MLP, train attention+output head
    freeze_prefixes = [
        "spectra_encoder.0.form_embedder_mod",   # 化学式嵌入（通用元素知识）
        "spectra_encoder.0.intermediate_layer",   # 早期MLP编码（通用特征提取）
        "spectra_encoder.1",                       # 碎片预测器（辅助任务）
    ]
