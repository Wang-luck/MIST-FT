...
class SpectraMolDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ========== 新增: 加载 MAGMA 文件 ==========
        self.magma_data = {}
        self.magma_modulo = kwargs.get("magma_modulo", 512)
        magma_folder = kwargs.get("magma_folder", None)
        if magma_folder is not None and kwargs.get("magma_aux_loss", False):
            magma_folder = Path(magma_folder)
            magma_tsv_dir = magma_folder / "magma_tsv"
            if magma_tsv_dir.exists():
                for magma_file in magma_tsv_dir.glob("*.magma"):
                    try:
                        df = pd.read_csv(magma_file, sep="\t")
                        if "frag_fp" in df.columns:
                            frag_fps = []
                            for val in df["frag_fp"].values:
                                if isinstance(val, str) and val:
                                    try:
                                        fp_list = [int(x) for x in val.split(",")]
                                        frag_fps.append(fp_list)
                                    except:
                                        frag_fps.append([])
                                else:
                                    frag_fps.append([])
                            self.magma_data[magma_file.stem] = frag_fps
                    except Exception as e:
                        logging.warning(f"Failed to load MAGMA file {magma_file}: {e}")
                logging.info(f"Loaded {len(self.magma_data)} MAGMA files")
            else:
                logging.warning(f"MAGMA tsv directory not found: {magma_tsv_dir}")
        # ========================================
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # ========== 获取 MAGMA 碎片指纹 ==========
        spec_name = spec.spectra_name
        magma_fps = self.magma_data.get(spec_name, [])
        
        # ========== 转换为 fingerprints 和 fingerprint_mask ==========
        if len(magma_fps) > 0:
            # 取每个碎片指纹的长度
            fp_len = max(len(fp) for fp in magma_fps if len(fp) > 0) if magma_fps else 0
            if fp_len > 0:
                fingerprints = torch.zeros((len(magma_fps), fp_len), dtype=torch.float32)
                fingerprint_mask = torch.ones(len(magma_fps), dtype=torch.float32)
                for i, fp in enumerate(magma_fps):
                    if len(fp) > 0:
                        fingerprints[i, :len(fp)] = torch.tensor(fp[:fp_len], dtype=torch.float32)
                    else:
                        fingerprint_mask[i] = 0
            else:
                fingerprints = torch.zeros((0, self.magma_modulo), dtype=torch.float32)
                fingerprint_mask = torch.zeros(0, dtype=torch.float32)
        else:
            fingerprints = torch.zeros((0, self.magma_modulo), dtype=torch.float32)
            fingerprint_mask = torch.zeros(0, dtype=torch.float32)
        # ==============================================================
...
class SpectraMolMismatchHDFDataset(SpectraMolDataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        batch["fingerprint_mask"] = fingerprint_mask
        batch["spec_name"] = spec_name
        # ===========================================
